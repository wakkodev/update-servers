...
perform_install() {
    ...
    if [[ -n "$_is_fresh_install" ]]; then
        echo
        echo -e "$(tbold)Congratulations! UDP Hysteria has been successfully installed on your server.$(treset)"
        #echo "Use 'agnudp' command to access the manager."

        echo
        echo -e "$(tbold)Client app HTTP Wakko:$(treset)"
        echo -e "$(tblue)https://play.google.com/store/apps/details?id=com.wakko.http$(treset)"
        echo
        echo -e "Follow me!"
        echo
        echo -e "\t+ Check out my website at $(tblue)https://wakkovpn-app.web.app/$(treset)"
        echo -e "\t+ Follow me on Telegram: $(tblue)https://t.me/wakkodev$(treset)"
        ...
        start_services
        echo
        echo -e "$(tbold) UDP Hysteria has been successfully updated to $VERSION.$(treset)"
        echo
    fi
}
...
perform_remove() {
    ...
    echo
    echo -e "$(tbold)Congratulations! UDP Hysteria has been successfully removed from your server.$(treset)"
    echo
